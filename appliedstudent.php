...
<section class="service-w3ls" id="service">
	...
	<h3 class="text-center"> Student Details </h3>
			...
			<tr><td>Name</td><td>Email</td><td>



			College</td>
			<td>Address</td><td>Scholarship</td><td>Caste</td><td>Religion</td><td>Percentage</td><td>Status</td><td>Verify</td></tr>
			
<?php 
include_once("dbconnect.php");


		$result1 = mysqli_query($conn, "SELECT * FROM scholarship_apply");

while($row1=mysqli_fetch_array($result1))
{$status=$row1['status'];
	$id=$row1['student_id'];
	$collegeid=$row1['clg_id'];
	$scholarid=$row1['scholarship_id'];
...
$address=$row3['address'];
		...
		$q4="select * from scholarship where id='$scholarid'";
		//	echo $q4;
	$result4=mysqli_query($conn,$q4);
	if($row4=mysqli_fetch_array($result4))
	{
$sc=$row4['id'];
$scholarship=$row4['name'];


	}
	$q5="select * from student where user_id='$id'";
			//echo $q3;
	$result5=mysqli_query($conn,$q5);
	if($row5=mysqli_fetch_array($result5))
	{

	$percentage=$row5['percentage_of_mark'];
				...
				<td><?php echo $scholarship;?></td>
					<td><?php echo $caste;?></td>
								<td><?php echo $religion;?></td>
											...
											<td><a href="verify.php?student=<?php echo $id;?>&&scholar=<?php echo $sc;?>">Verify</a></td>
</tr>
...
?>
		...
		<center><br/><b><a class="linkA" href="scholarship_officer.php">Back</a></b></center>
	</div>
</section>
<!-- /services section -->
