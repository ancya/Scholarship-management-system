...
<section class="contact-wthree jarallax" id="contact">
	<div class="container">
		<h3 class="text-center slideanim">REGISTER HERE</h3>
		...
		<div class="col-lg-6 col-md-6 col-sm-6 contact-agile2 slideanim">
			...
			<div class="subscribe"><center>
				<form action="#" method="post">
					...
					<div>
						<input id="email1" type="number" name="phonenumber" placeholder="Enter phonenumber" required="">
					</div>
					<div>
						<input  type="email" name="email" placeholder="enter email" required="">
					</div>
					<div class="form-group2 slideanim">
						<button class="btn btn-outline btn-lg" type="submit" name="submit">REGISTER</button>
					</div>
...
if(isset($_POST["submit"]))
{
 ...
 $email=$_POST['email'];
$result=mysqli_query($conn,"SELECT * FROM usertb WHERE username='$username'");
 ...
 else if($count==1)
{
	...
	if(mysqli_query($conn,"INSERT INTO usertb(username,password,email,name,usertype,phonenumber) VALUES('$username','$password','$email',' $f_name','$role','$number')"))
		{	
			$result1=mysqli_query($conn,"select * from usertb order by id desc limit 1");
			$rwo=mysqli_fetch_assoc($result1);
			$Uid=$rwo['id'];
			$_SESSION['user']=$Uid;
			if(mysqli_query($conn,"INSERT INTO student(user_id) VALUES('$Uid')")){
			echo "<script>alert('Success. click ok to continue..');
